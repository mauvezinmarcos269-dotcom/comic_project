...
from core.constants import INVALID_VALUES
from core.features.advanced_analytics import extract_title_keywords, perform_regression
from core.ui.export_utils import REPORTLAB_AVAILABLE, generate_dashboard_pdf
...
def get_top_entity(d: pd.DataFrame, col: str) -> tuple[str, int]:
    ...
    if col not in d.columns or d.empty:
        return "N/A", 0
    valid_mask = ~d[col].astype(str).str.strip().str.lower().isin(INVALID_VALUES)
...
""")
    ...
    with dl2:
        if REPORTLAB_AVAILABLE:
            ...
            st.caption("💡 终端执行 `pip install reportlab` 即可解锁 PDF 导出")

    st.divider()

    # ── 分析标签页（已移除原 PCA Tab，避免与 tab3 聚类探针重复）──────────────
    tabs = st.tabs([
        "🔥 特征热力图",
        "📈 时间序列趋势",
        "🧮 销售回归",
    ...
    with tabs[0]:
        ...
        fig = plot_correlation_heatmap(filtered_df, method)
        if fig:
            st.plotly_chart(fig, use_container_width=True)

    with tabs[1]:
        fig = plot_time_series_trend(filtered_df)
        if fig:
            st.plotly_chart(fig, use_container_width=True)
    ...
    with tabs[3]:
        fig = plot_word_freq(get_keywords_cached(filtered_df))
        if fig:
            st.plotly_chart(fig, use_container_width=True)

    with tabs[4]:
        try:
            fig = plot_decade_top10(filtered_df)
            if fig:
                st.plotly_chart(fig, use_container_width=True)
        except Exception:
    ...
    with tabs[5]:
        ...
        with c1:
            if "Writer" in filtered_df.columns:
                fig = plot_top_creators(filtered_df, "Writer", 20)
                if fig:
                    st.plotly_chart(fig, use_container_width=True)
        with c2:
            if "Artist" in filtered_df.columns:
                fig = plot_top_creators(filtered_df, "Artist", 20)
                if fig:
                    st.plotly_chart(fig, use_container_width=True)
    ...
    with tabs[6]:
        c1, c2 = st.columns(2)
        with c1:
            fig = plot_marvel_vs_dc_trend(filtered_df)
            if fig:
                st.plotly_chart(fig, use_container_width=True)
        with c2:
            fig = plot_publisher_share(filtered_df)
            if fig:
                st.plotly_chart(fig, use_container_width=True)